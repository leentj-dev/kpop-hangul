...
1. lrclib.net에서 LRC 파싱 → 가사 전체에서 한국어 단어를 가능한 많이 추출 (곡 흐름 순서대로, 중복 포함)
...
## 단어 규칙
- 가능한 많은 단어 (20개 상한 없음) — 곡 처음부터 끝까지 가사 흐름을 따라 촘촘히 커버
- **중복 단어 허용/권장**: 같은 단어가 뒤에서 또 나오면 그 등장 타임스탬프마다 각각 별도 카드로 추가 (앞에서 나왔다고 dedup 금지 — 뒤쪽 싱크가 비지 않게)
